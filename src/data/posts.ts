...
export async function getAllPosts(): Promise<BlogPost[]> {
  try {
    return Object.entries(posts).map(([path, content]) => {
      ...
      // Use excerpt from front matter if available, otherwise leave it undefined
      // As per request: "if there is no excerpt in frontmatter, leave the meta description empty"
